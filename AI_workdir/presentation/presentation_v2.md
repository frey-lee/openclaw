...
## Slide 4: Key Concepts — Sessions

Sessions are persistent and long-lived — many exist across different channels simultaneously. The session store maps keys to session IDs:

```
agent:main:telegram:12345    →  session-abc  →  abc.jsonl
agent:main:whatsapp:67890    →  session-def  →  def.jsonl
agent:main:cli               →  session-ghi  →  ghi.jsonl
```

Same channel + user resolves to the same session — effectively "always on" for that key.

---

## Slide 5: Key Concepts — The Agent Loop
...
- Tool schemas are normalized per LLM provider to save tokens (e.g. stripping validation keywords that Gemini doesn't support)
...
## Slide 9: Subagents — Orchestration
...
## Slide 10: Subagents — v2026.2.17 Changes
...
## Slide 12: Memory — Write

Three writers, three triggers:
...
workspace/
  ...
  │  ├─ 2026-02-01.md         ← daily notes (memory flush)
  │  └─ ...
...
| Query Expansion| FTS fallback if no embedding API key: strips stopwords, uses remaining |
...
FTS query expansion example:
`"That thing we discussed about the API"` → strips stopwords → keywords: `["discussed", "API"]`
...
"When X happens, also do Y" — 7 categories, ~20 hook points:
