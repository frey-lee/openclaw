...
//
// All built-in channel implementations have been extracted to external plugins.
// Channel docks are now populated exclusively through the plugin registry.
const DOCKS: Partial<Record<ChatChannelId, ChannelDock>> = {};

export function buildDockFromPlugin(plugin: ChannelPlugin): ChannelDock {
  return {
    id: plugin.id,
...
export function listChannelDocks(): ChannelDock[] {
  // Collect static DOCKS entries (if any) keyed by core channel order.
  const baseEntries: Array<{ id: string; dock: ChannelDock; order?: number }> = [];
  for (const id of CHAT_CHANNEL_ORDER) {
    const dock = DOCKS[id];
    if (dock) {
      baseEntries.push({ id, dock, order: getChatChannelMeta(id).order });
    }
  }

  // Collect plugin-registered docks (includes core channels registered via plugins).
  const pluginEntries = listPluginDockEntries();

  // Deduplicate: plugin entries that overlap with static DOCKS are skipped.
  const seenIds = new Set(baseEntries.map((e) => e.id));
  const combined = [...baseEntries];
  for (const entry of pluginEntries) {
    if (!seenIds.has(String(entry.id))) {
      combined.push({ id: String(entry.id), dock: entry.dock, order: entry.order });
